...
## 2026-04-22 - Batch Inserts to prevent N+1 queries during plan creation\n**Learning:** Found an N+1 query issue in the billing plan API (`src/billing/handler/plan.go`) where plan prices were being queried individually for each price within a loop.\n**Action:** Replaced the loop creating individual entries with a batch creation pattern mapping the struct and slicing it first: `pricesToInsert := make([]billing_entity.PlanPrice, 0, len(body.Prices))` and performing a single database insertion with GORM: `tx.Create(&pricesToInsert)`.
